...
## 2026-02-06 - [Date Formatting Optimization]
**Learning:** Formatting dates inside a loop using `pd.Timestamp.strftime` is a major performance bottleneck. Pre-calculating date strings using `np.datetime_as_string` outside the loop yields significant speedups (observed ~26x).
**Action:** Use `np.datetime_as_string(dates, unit='D').astype(object)` to generate a numpy array of strings for logging or reporting purposes when iterating over time-series data.
