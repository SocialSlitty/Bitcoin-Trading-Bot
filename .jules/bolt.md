...
## 2026-01-30 - [Date Formatting in Loops]
**Learning:** Formatting `pd.Timestamp` to strings inside a loop using `.strftime()` is extremely slow. Using `np.datetime_as_string` on the entire array is orders of magnitude faster (~15x speedup observed).
**Action:** Always pre-calculate date strings using `np.datetime_as_string(dates, unit='D').astype(object)` before entering a loop that requires string representations of dates.
