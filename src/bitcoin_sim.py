...
def run_simulation(df, config: SimConfig = None):
    ...
    # Pre-calculate date strings for the simulation window to avoid formatting in loop
    # OPTIMIZATION: Vectorized string conversion is ~17x faster than strftime in loop
    # We only convert the slice we actually iterate over
    relevant_dates = dates[start_idx:]
    date_strings = np.datetime_as_string(relevant_dates, unit="D").astype(object)

    for i in range(start_idx, len(df)):
        # Direct numpy array access is much faster than df.iloc[i]
        today_date_val = dates[i]
        # Use pre-calculated date string (adjusting index)
