...
def run_simulation(df, config: SimConfig = None):
    ...
    # OPTIMIZATION: Pre-calculate date strings vectorially to avoid expensive loop formatting
    # This is ~10x faster than formatting dates inside the loop.
    ...
    for i in range(start_idx, len(df)):
        ...
        # Use pre-calculated date string (much faster)
        current_date = str(date_strings[i])
